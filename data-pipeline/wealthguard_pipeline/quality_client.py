...
def build_validate_payload(
    ...
    """The exact JSON body ``POST /api/v1/validate`` expects.

    Public (not just used by :meth:`QualityEngineClient.validate`) so the
    React frontend's demo fixture -- which calls the Java engine directly
    from the browser instead of through this client -- can be generated by
    reusing the same column mapping, rather than re-deriving snake_case ->
    camelCase by hand in a second place. See
    ``wealthguard_pipeline.seed.export_frontend_fixture``.
    """
    return {
...
class QualityEngineClient:
    ...
    def validate(
        ...
        """
        payload = build_validate_payload(positions, clients, instruments, target_allocations, evaluation_date)
